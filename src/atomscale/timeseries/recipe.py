...
from atomscale.results.recipe import RecipeResult
...
class RecipeProvider(TimeseriesProvider[RecipeResult]):
    TYPE = "recipe"

    # Recipe (process-plan) timeseries share tool-state's property-centric
    # payload shape, so parsing is identical; only the endpoint and the result
    # type differ. Property values keep their API names; only the time columns
    ...
    def fetch_raw(self, client: BaseClient, data_id: str) -> Any:
        # Recipe has its own endpoint, distinct from tool-state: a recipe is the
        # process *plan* (setpoints), tool-state is the observed sensor readback.
        return client._get(sub_url=f"recipe/{data_id}/timeseries/")
