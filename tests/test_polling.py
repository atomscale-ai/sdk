...
def test_iter_poll_yields_max_polls(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
...
def test_iter_poll_dedupes_by_key(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    monkeypatch.setattr(time, "sleep", lambda *_: None)
...
def test_iter_poll_until_predicate(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    monkeypatch.setattr(time, "sleep", lambda *_: None)
...
def test_iter_poll_on_error_and_continue(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    monkeypatch.setattr(time, "sleep", lambda *_: None)
    ...
    results = list(
        iter_poll(
            client, data_id,
            interval=0.01, max_polls=2,  # first raises, second succeeds
...
def test_iter_poll_jitter_uses_interval_bound(
    ...
    it = iter_poll(
        client, data_id,
        interval=0.2, jitter=999.0, max_polls=2,  # clamp jitter to interval
    )
    next(it); next(it)  # consume
...
def test_iter_poll_with_fixture_result_payload(
    ...
    monkeypatch.setattr(time, "sleep", lambda *_: None)
    provider = SeqProvider([{"rev": 1, "payload": result}, {"rev": 2, "payload": result}])
    monkeypatch.setattr("polling.get_provider", lambda name: provider)

    out = list(iter_poll(client, data_id, interval=0.01, max_polls=2, distinct_by=lambda r: r["rev"]))
...
@pytest.mark.asyncio
async def test_aiter_poll_yields_max_polls(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
...
@pytest.mark.asyncio
async def test_aiter_poll_dedupes(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    async def fast_sleep(_): return None
    monkeypatch.setattr(asyncio, "sleep", fast_sleep)
...
@pytest.mark.asyncio
async def test_aiter_poll_on_error_and_continue(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    async def fast_sleep(_): return None
    monkeypatch.setattr(asyncio, "sleep", fast_sleep)
...
@pytest.mark.asyncio
async def test_start_polling_task_awaits_on_result(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    async def fast_sleep(_): return None
    monkeypatch.setattr(asyncio, "sleep", fast_sleep)
...
def test_start_polling_thread_stops_with_event(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    monkeypatch.setattr(time, "sleep", lambda *_: None)
...
def test_iter_poll_fire_immediately_smoke(monkeypatch: pytest.MonkeyPatch, client: Client, data_id: str):
    ...
    out1 = list(iter_poll(client, data_id, interval=0.01, max_polls=1, fire_immediately=True))
    assert out1 == [{"x": 1}]

    # False
    monkeypatch.setattr("polling.get_provider", lambda name: provider2)
    out2 = list(iter_poll(client, data_id, interval=0.01, max_polls=1, fire_immediately=False))
    assert out2 == [{"y": 1}]
